...
bool AABB::intersects(const AABB& other) const {
    ...
    return std::abs(displacement.x) < (other.half_dimension.x + half_dimension.x) && std::abs(displacement.y) < (other.half_dimension.y + half_dimension.y);
}
...
bool QuadTree::insert(Object* object){
    ...
    // otherwise subdivide insert into 1 of the children
    if (north_west == nullptr){
        subdivide();
    }
    return north_west->insert(object) 
